...
# ============================================
# 8. Shell-level settings restore (from action.sh block [0])
# ============================================
echo "[8] Shell-level settings (restore defaults)"

# 恢复 GPU 渲染为默认（硬件渲染）
settings delete global force_gpu_rendering 2>/dev/null && echo "  force_gpu_rendering deleted"

# 恢复省电模式为默认
settings delete global power_save_mode 2>/dev/null
settings delete global low_power 2>/dev/null
...
# 恢复严格模式（默认关闭）
settings delete global strict_mode_enabled 2>/dev/null

# 恢复 MSAA（默认关闭）
settings delete global multisampling_enabled 2>/dev/null
...
# 恢复指针速度为默认（1.0）
settings put system pointer_speed 1 2>/dev/null && echo "  pointer speed -> 1"

# 恢复 dex2opt 后台优化（启用）
pm enable com.android.backgrounddexoptjob/.BackgroundDexOptJobService 2>/dev/null && echo "  background dexopt job enabled"

# 恢复 GPU 渲染白名单（默认空）
settings delete global gpu_renderer_apps 2>/dev/null

echo "  Shell-level settings restored (some may require reboot)"

# ============================================
# 9. 其他未备份的修改（IO、显示、触控等）无法精确恢复，给出提示
# ============================================
echo ""
